...
### Supabase 동기화 (Phase 3~5)

- [x] 12. Google 로그인 — sign-in 화면 → Google 버튼 → in-app browser → 계정 선택 → 앱 복귀 → 옷장 진입 (PKCE flow)
- [x] 13. 옷 sync — 옷 등록 → 옷장 화면 진입 시 자동 push → Supabase `clothes` 테이블에 row 생성 (image_url 은 `{user_id}/{uuid}.jpg`)
- [x] 14. 피팅 sync — 피팅하기 → 합성 결과 저장 → 옷장 진입 시 `fittings` 테이블에 row 생성. tryOn 호출 시 imageUri 가 remote path 여도 signed URL 로 자동 변환되어 multipart 정상.
- [x] 15. 전신사진 sync — 전신사진 등록 → 옷장 진입 시 `user_profiles.person_image_url` 갱신. reload 후에도 사진 유지.
- [ ] 16. **로그인 + RLS 격리**: A 계정 옷 등록 → 로그아웃 → B 계정 로그인 → 옷장 빈 상태 확인.
- [ ] 17. **재설치 복원**: 앱 데이터 삭제/재설치 → 같은 계정 로그인 → 옷장/피팅/전신사진 모두 복원.
- [ ] 18. **로그아웃 캐시 클리어**: 로그아웃 직후 AsyncStorage 비워지고, sign-in 으로 강제 이동.
- [ ] 19. **LWW**: 두 기기에서 같은 옷의 메타를 다르게 수정 → 더 늦은 `updated_at` 이 최종 값.
- [ ] 20. **soft delete 전파**: 기기 1 에서 옷 삭제 → 기기 2 sync 후 사라짐. 그 옷을 사용한 피팅은 placeholder (ADR-008).
- [ ] 21. **비행기 모드 큐잉**: 오프라인에서 옷 등록(로컬 즉시 표시) → 온라인 복귀 시 자동 push (현재는 다음 focus 진입 시 push, netinfo 자동화는 후속).
